...
// supabase/functions/verifyRecaptcha/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    // שליפת המפתח הסודי מהגדרות הסביבה של סופאבייס (אסור לחשוף אותו!)
    ...
    // 3. בדיקת הציון (Score)
    // גוגל מחזירה ציון בין 0.0 (בוט) ל-1.0 (אנושי). 0.5 זה רף הגיוני להפרדה.
    const isHuman = recaptchaData.success && recaptchaData.score >= 0.5;

    if (!isHuman) {
      // אם זה בוט, מחזירים success: false ל-React
      return new Response(
