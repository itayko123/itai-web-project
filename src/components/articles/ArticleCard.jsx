  ...
  return (
    <Link to={`/articles/${article.id}/${slug}`} className="group block bg-card border border-border rounded-2xl overflow-hidden hover:shadow-lg hover:border-primary/30 transition-all duration-300">
      ...
      <div className="p-4">
        ...
        <h3 className="font-bold text-sm text-foreground group-hover:text-primary transition-colors line-clamp-2 mb-1.5 leading-snug">{article.title}</h3>
        {article.excerpt && <p className="text-xs text-muted-foreground line-clamp-2 mb-3 leading-relaxed">{article.excerpt}</p>}
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            {article.read_time_minutes && (
              <span className="flex items-center gap-0.5"><Clock className="w-3 h-3" />{article.read_time_minutes} דק׳</span>
            )}
          </div>
