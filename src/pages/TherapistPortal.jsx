...
import {
  ...
  TREATMENT_METHOD_GROUPS,
} from "@/lib/therapyOptions";
...
const isTreated = (status) =>
  ["treated", "responded", "done", "closed"].includes(normalizeStatus(status));
...
const formatRequestDate = (r) => {
  const raw = getRequestDate(r);
  if (!raw) return "תאריך לא זמין";

  const d = new Date(raw);
  if (Number.isNaN(d.getTime())) return "תאריך לא זמין";
...
export default function TherapistPortal() {
  ...
  const { data: requests = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return rows.sort((a, b) => {
        const da = new Date(getRequestDate(a) || 0).getTime();
        const db = new Date(getRequestDate(b) || 0).getTime();
        return db - da;
      });
    },
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4 text-center">
        ...
        <p className="text-muted-foreground text-sm">
          יש להתחבר כדי לגשת לאזור המטפלים.
        </p>
  ...
  if (!therapist) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4 text-center">
        ...
        <p className="text-muted-foreground text-sm">
          הכתובת {user.email} אינה משויכת לפרופיל מטפל במערכת.
        </p>
        <Button asChild variant="outline">
          <a href="/register-therapist">הירשם כמטפל</a>
        </Button>
      </div>
    );
  }

  const pendingCount = requests.filter((r) => !isTreated(r.status)).length;
  ...
  const handleSave = () => {
    updateMutation.mutate({
      ...editData,
      price_per_session: editData.price_per_session
        ? Number(editData.price_per_session)
        : undefined,
      years_experience: editData.years_experience
        ? Number(editData.years_experience)
        : undefined,
      formats: editFormats,
  ...
  const handleLicenseUpload = async (e) => {
    ...
    await supabase
      .from("Therapist")
      .update({ license_document_url: fileUrl })
      .eq("id", therapist.id);
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-5 border-b border-border overflow-x-auto">
        {tabs.map((t, i) => (
          ...
          >
            {t} {i === 0 && pendingCount > 0 && (
              <span className="mr-1.5 bg-primary text-white text-xs px-1.5 py-0.5 rounded-full">
                {pendingCount}
              </span>
            )}
          </button>
      ...
      {tab === 0 && (
        <div className="space-y-3">
          ...
          {requests.map((r) => {
            const treated = isTreated(r.status);

            return (
              <div
                key={r.id}
                className={`bg-card border rounded-xl p-4 ${
                  treated ? "border-border" : "border-primary/30"
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        variant={treated ? "secondary" : "default"}
                        className="text-xs"
                      >
                        {treated ? "טופל" : "חדש"}
                      </Badge>
                    ...
                    {r.message && (
                      <p className="text-sm bg-muted/40 rounded-lg p-2 mt-1">
                        {r.message}
                      </p>
                    )}
                  ...
                  {!treated && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => markTreated.mutate(r.id)}
          ...
          {editMode ? (
              ...
              <div className="space-y-4">
                ...
                {checkboxGroup(
                  ...
                  [
                    ...
                    { value: "clal_insurance", label: "כלל ביטוח" },
                ...
                <div className="bg-muted/30 rounded-xl p-4 space-y-3 border border-border">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        יופיע בפרופיל הציבורי שלך
                      </p>
                    </div>
                    <Switch
                      checked={editData.immediate_availability}
                      onCheckedChange={(v) =>
                        setEditData((d) => ({
                          ...d,
                          immediate_availability: v,
                        }))
                      }
                    />
                  </div>
                  <div
                    className={`text-xs font-medium px-3 py-1.5 rounded-lg inline-flex items-center gap-1.5 ${
                      editData.immediate_availability
                        ? "bg-emerald-100 text-emerald-700"
                        : "bg-muted text-muted-foreground"
                    }`}
                  >
                    {editData.immediate_availability
                      ? "✓ מוצג כזמין למטופלים חדשים"
                      : "✗ מוצג כלא זמין כעת"}
                  </div>
                </div>
                ...
                <div className="border border-dashed border-border rounded-xl p-4">
                  <Label className="text-sm font-medium">
                    העלאת מסמך רישיון מקצועי
                  </Label>
                  <p className="text-xs text-muted-foreground mt-1 mb-3">
                    העלה/י תעודת הסמכה או רישיון מקצועי לאימות
                  </p>
                  {therapist.license_document_url && (
                    <div className="flex items-center gap-2 text-xs text-primary mb-2">
                      <FileText className="w-4 h-4" />
                      <a
                        href={therapist.license_document_url}
                        target="_blank"
                        rel="noreferrer"
                        className="underline"
                      >
            ...
            <div className="space-y-3 text-sm">
              {[
                ...
                {
                  label: "מחיר לפגישה",
                  value: therapist.price_per_session
                    ? `₪${therapist.price_per_session}`
                    : null,
                },
                {
                  label: "שנות ניסיון",
                  value: therapist.years_experience
                    ? `${therapist.years_experience} שנים`
                    : null,
                },
                ...
                (item) =>
                  item.value && (
                    <div key={item.label} className="flex gap-2">
                      <span className="font-medium text-muted-foreground text-xs w-28 flex-shrink-0">
                        {item.label}:
                      </span>
      ...
      {/* Tab: Articles */}
      {tab === 2 && therapist && (
        <PortalArticles therapistId={therapist.id} therapistName={therapist.full_name} />
      )}
    </div>
  );
}